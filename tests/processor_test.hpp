...
namespace cycfi { namespace infinity
{
   ...
   // (the buffer size). With a sampling rate of 32kHz and a buffer size of 
   // 1024 samples, the delay will be 32ms. This delay is the roundtrip ADC 
   // to DAC latency.
   ////////////////////////////////////////////////////////////////////////////
   template <
      typename Processor
   ...
   struct mono_processor : Processor
   {
      ...
      void start()
      {
         _adc.template enable_channel<1, port::portc + 0, 1>();
         _adc.start();
         _clock.start();
